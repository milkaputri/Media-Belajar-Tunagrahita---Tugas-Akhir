...
function renderQuestion(){
  ...
  if(q.type === "dragdrop_number_to_box"){
    ...
    rendererController = renderBottleDecimalPick({
      mount: questionBox,
      q,
      onAnswerChange: (v) => { currentAnswer = v; }
    });
    ...
    rendererController = renderPercentColor100({
      mount: questionBox,
      q,
      onAnswerChange: (v) => { currentAnswer = v; }
    });
...
function generateBil20Mixed(){
  ...
  const n1 = 7;
  const n2 = 7;
  const n3 = DEFAULT_QUESTIONS - n1 - n2;
...
function generateDesimalBotolAir(){
  const list = [];
  for(let i=0;i<DEFAULT_QUESTIONS;i++){
    list.push(genDesimal_BotolAir());
  }
  return shuffle(list);
}
...
function generateAllGamesOnce(){
  return shuffle([
    ...
    genBil20_TarikGaris(),
    genBil20_Urutkan(),
    genBil50_HitungObjek(),
    genNilaiTempat_NilaiGambar(),
    genUang20_KenalUang(),
    genUang20_BelanjaBarang(),
    genUang50_BenarTidak(),
    genUang50_CelenganKu(),
    genBil50_TambahMobil(),
    genBil50_KurangApel(),
    genBil50_BagiDonat(),
    genBil50_KaliKursi(),
...
function generateBeratRinganPilihBeratGame(){
  const list = [];
  for(let i=0;i<DEFAULT_QUESTIONS;i++){
    list.push(genBeratRingan_PilihBerat());
  }
  return shuffle(list);
}
...
function generateRotatingBilanganGame(){
  ...
  return generateBil20Mixed();
}
// ---------- INIT
function init(){
  questions = generateRotatingBilanganGame();
